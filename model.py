...
class BIODGI(nn.Module):
    ...
    def forward(self, attrs, adjs):
        pos_embeddings = [self.models[i](attrs[i], adjs[i])
                          for i in range(self.num_views)]
        neg_embddings = [self.models[i](gen_negative_samples(
            attrs[i]), adjs[i]) for i in range(self.num_views)]
    ...
    def embed(self, attrs, adjs):
        embeddings = [self.models[i](attrs[i], adjs[i])
                      for i in range(self.num_views)]
